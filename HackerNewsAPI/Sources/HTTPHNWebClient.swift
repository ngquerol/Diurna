...
public class HTTPHNWebClient {
    ...
    private var userCookie: HTTPCookie? {
        guard
            let cookie = HTTPCookieStorage.shared.cookies(for: HackerNewsWebpage.login.path)?.last
        else {
            return nil
        }
    ...
    private func performLoginRequest(
        ...
        var request = URLRequest(url: HackerNewsWebpage.login.path)
    ...
    private func verifyLoginCookie() -> HNWebResult<Void> {
        ...
        guard
            cookie.domain == HackerNewsWebpage.login.path.host,
            cookie.path == "/",
            cookie.name == "user",
